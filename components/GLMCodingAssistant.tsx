// components/GLMCodingAssistant.tsx
...
import { Copy, CheckCircle, AlertCircle, Code, Zap, Bug, TrendingUp, TestTube } from 'lucide-react';
...
type ActionType = 'generate' | 'refactor' | 'explain' | 'debug' | 'optimize' | 'generate-tests';
...
export default function GLMCodingAssistant() {
  ...
  const checkConfiguration = async () => {
    try {
      ...
      if (!data.configured) {
        setApiError(data.error || 'GLM service is not configured');
      }
    } catch (error) {
      setIsConfigured(false);
      setApiError('Failed to check GLM service configuration');
    }
  };
    ...
    return (
      <Card className="w-full max-w-4xl mx-auto">
        <CardContent className="p-6">
          <ErrorMessage message={apiError || 'GLM service is not configured'} />
        </CardContent>
      </Card>
    );
  }
